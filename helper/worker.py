...
async def encc(e):
    try:
        ...
        cmd = f'ffmpeg -i "{dl}" -c:v libaom-av1 -crf 22 -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? "{out}" -y'
        process = await asyncio.create_subprocess_shell(
...
async def sample(e):
    ...
    ncmd = f'ffmpeg -i "{dl}" -ss {ss} -to {dd} -c:v libaom-av1 -crf 22 -map 0:v -c:a aac -map 0:a -c:s copy -map 0:s? "{out}" -y'
    process = await asyncio.create_subprocess_shell(
        ncmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
